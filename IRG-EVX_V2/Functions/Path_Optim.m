...
if PathFun3D(init,z,w,t,init,x_left,y_left,x_right,y_right) > 0
%     fun = @(v) sum((abs(Curvature(v)./min(Curvature(init)))).^2);
    fun = @(v) PathFun3D(v,z,w,t,init,x_left,y_left,x_right,y_right);   
    A = [];
    b = [];
    Aeq = [];
    beq = [];
    ...
    opts = optimset('Display',Display,'Algorithm','interior-point', 'MaxIter', Inf, 'MaxFunEvals', Inf...
        ,'TolCon',1e-100,'TolX',1e-25,'TolFun',1e-100,'UseParallel',Parallel);
