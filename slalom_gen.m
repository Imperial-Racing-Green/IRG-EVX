function [coords]=slalom_gen(A,cone_dist,cone_number,x1,y1,x,y,dist,slalom_number)
...
k1 = find(Diff == min(Diff));

init_dist = dist(k1);
final_dist = init_dist + slalom_length;

% k2 = find((abs(dist-final_dist))<0.5);
DistDiff = abs(dist-final_dist);
k2 = find(DistDiff == min(DistDiff));
